...
import { ChemicalAssessment, Company, Equipment, Responsible } from '../types';
...
import { generateChemicalPDF } from '../lib/pdfGenerator';
...
interface ChemicalFormProps {
  onSave: (assessment: ChemicalAssessment) => void;
  onCancel: () => void;
  initialData?: Partial<ChemicalAssessment>;
}

const CHEMICAL_AGENTS = [
  'Poeira Total', 'Varredura Metal', 'Varredura VO', 'Ozona (Ozônio)', 'Ác. Clorídrico',
  'Poeira Respirável', 'Manganês', 'Tolueno', 'Negro de Fumo', 'Ác. Sulfúrico',
  'Sílica', 'Ferro', 'Xileno', 'Níquel', 'Amônia',
  'Algodão', 'Cromo', 'Etilbenzeno', 'Estireno', 'Cloro',
  'Madeira', 'Chumbo', 'Álcool Etílico', 'n-hexano', 'Benzeno'
];

export function ChemicalForm({ onSave, onCancel, initialData }: ChemicalFormProps) {
  ...
  const [formData, setFormData] = useState<Partial<ChemicalAssessment>>({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setFormData(prev => ({ ...prev, samplingTime: metrics.elapsed }));
    }, 1000);
  ...
  const handleChange = (field: keyof ChemicalAssessment, value: any) => {
  ...
  const toggleAgent = (agent: string) => {
    const current = formData.agents || [];
    if (current.includes(agent)) {
      handleChange('agents', current.filter(a => a !== agent));
    } else {
      handleChange('agents', [...current, agent]);
    }
  };
  ...
  const validateForm = (status: 'open' | 'completed') => {
    if (status === 'completed') {
      ...
      const missing = required.filter(f => !formData[f as keyof ChemicalAssessment]);
      if (missing.length > 0) {
  ...
  const handleSave = async (status: 'open' | 'completed' = 'open') => {
    ...
    try {
      ...
      await onSave(finalData as ChemicalAssessment);
    } finally {
  ...
  const handleDownload = () => {
    generateChemicalPDF(formData as ChemicalAssessment);
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            <Label>Agentes Monitorados</Label>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
              {CHEMICAL_AGENTS.map(agent => (
                <div key={agent} className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor={agent} className="text-xs font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                    {agent}
                  </label>
                </div>
              ))}
            ...
            <div className="space-y-2">
              <Label htmlFor="otherAgents">Outro (especificar)</Label>
              <Input id="otherAgents" value={formData.otherAgents || ''} onChange={e => handleChange('otherAgents', e.target.value)} />
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2 md:col-span-2">
            ...
            >
              ...
              {equipments.filter(e => e.type === 'chemical').map(e => (
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-2">
            <Label htmlFor="flowInit">Vazão Inicial (l/min)</Label>
            <Input id="flowInit" value={formData.flowInitial || ''} onChange={e => handleChange('flowInitial', e.target.value)} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="flowFinal">Vazão Final (l/min)</Label>
            <Input id="flowFinal" value={formData.flowFinal || ''} onChange={e => handleChange('flowFinal', e.target.value)} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="volume">Volume (litros)</Label>
            <Input id="volume" value={formData.volume || ''} onChange={e => handleChange('volume', e.target.value)} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="samplingTime">Tempo Amostragem (Auto)</Label>
            <Input id="samplingTime" value={formData.samplingTime || ''} readOnly className="bg-slate-50 font-mono" />
          </div>
      ...
      {(formData.isStarted || formData.status === 'completed') && (
        <ReportPreview data={formData as ChemicalAssessment} type="chemical" />
      )}
